...
class histori_kematian_noniden extends CI_Model {
...
public function getDefaultJumlah($primary_key){
    ...
    $this->db->from("histori_kematian_noniden");
    $this->db->where("id_histori_mati", $primary_key);
    $query = $this->db->get();	
    $result = $query->result();
...
public function jumlahKematian($kode_lk = null){
/*
...
FROM `histori_kematian_noniden`
WHERE id_satwa_noniden LIKE "tsi1%"
