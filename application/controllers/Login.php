...
$loginConfig = array(
	//"Page After Login" => "/",
	"Page After Login" => "lemkon",
	//"Page After Login" => "php",
	"Error Message" => "Username atau Password Anda salah. Silakan input kembali Username dan Password Anda.",
	//"Use MD5 Encryption" => false,
        "Use MD5 Encryption" => true,
...
class Login extends CI_Controller {
	...
	function index() {
		...
		if ($this->db->table_exists('crud_users')){
			...
			echo "SYSTEM REQUIREMENT: SQL TABLE crud_users DOESN'T EXISTS BUT... Hey, we can do it for you, if the database connection is configured correctly ;)<p><a href=\"".site_url('login/createDBTable')."\"><button>Create the required table in your MySQL database</button></a></p>Oh, a user 'admin' (password 'admin') will be create too, so you can log in directly! Just click on the button!";
	...
	function createDBTable(){
		if ($this->db->table_exists('crud_users')){
			...
			echo '<p><a href="'.site_url('login').'"><button>Go to the Login Page</button></a></p>'; 
		}
	}
	...
	public function manage_permissions(){
		if(!$this->login_model->isLogged()){ redirect(site_url("login")); return;}
        ...
        $crud->add_action('Reset Password', '', 'login/resetPass','ui-icon-lightbulb','');
